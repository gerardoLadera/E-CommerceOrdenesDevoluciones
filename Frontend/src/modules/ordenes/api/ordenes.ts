...
import { API_ORDERS_QUERY, API_UPDATE } from "./api";
...
export const getOrdenes = async (params: {
  ...
  try {
    const response = await API_ORDERS_QUERY.get(apiUrl);
...
export const getOrdenById = async (idOrden: string) => {
  try {
    const response = await API_ORDERS_QUERY.get(`/api/orders/${idOrden}`);
