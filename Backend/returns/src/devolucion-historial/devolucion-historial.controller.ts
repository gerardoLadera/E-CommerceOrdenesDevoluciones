...
export class DevolucionHistorialController {
  constructor(private readonly devolucionHistorialService: DevolucionHistorialService) {}
  ...
  @Patch(':id')
  update(@Param('id') id: string, @Body() updateDevolucionHistorialDto: UpdateDevolucionHistorialDto) {
    return this.devolucionHistorialService.update(+id, updateDevolucionHistorialDto);
  }
